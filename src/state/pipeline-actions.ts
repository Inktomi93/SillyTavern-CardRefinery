...
/**
 * Execute refinement: re-run rewrite using the latest analyze feedback.
 *
 * Refinement is a special mode where:
 * 1. The rewrite stage runs again with isRefinement=true
 * 2. The analyze result is included as feedback to guide improvements
 * 3. The iteration counter is incremented
 *
 * @param state - Current popup state (will be mutated)
 * @param options - Execution options
 * @returns The rewrite result, or null if execution couldn't start
 */
export async function executeRefinementAction(
    state: PopupState,
    options: { callbacks?: PipelineCallbacks } = {},
): Promise<StageResult | null> {
    const { callbacks } = options;

    // Validation
    if (!state.character) {
        log.warn('Cannot execute refinement: no character selected');
        return null;
    }

    if (state.isGenerating) {
        log.warn('Cannot execute refinement: generation already in progress');
        return null;
    }

    // Refinement requires a previous analyze result
    if (!state.stageResults.analyze) {
        log.warn('Cannot execute refinement: no analyze result available');
        return null;
    }

    // Build context for refinement (rewrite with isRefinement flag)
    const context = buildStageContext(state, 'rewrite');
    if (!context) return null;

    // Set refinement flag - this tells prompt builder to include analyze feedback
    context.isRefinement = true;
    ...
    try {
        ...
        recordResultInState(state, result);
        callbacks?.onStageComplete?.('rewrite', result);

        if (result.error) {
            callbacks?.onError?.('rewrite', result.error);
        }
        ...
        const errorMsg =
            error instanceof Error ? error.message : 'Unknown error';
        log.error('Refinement failed:', error);

        setStageStatusInState(state, 'rewrite', 'error');
        callbacks?.onError?.('rewrite', errorMsg);
